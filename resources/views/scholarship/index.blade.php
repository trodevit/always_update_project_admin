...
@section('content')
    <div class="container mt-5">
        <h2 class="mb-4">Scholarship Table</h2>
...
{{--                <td><a href="{{ asset($course->pdf) }}" target="_blank">Download PDF</a></td>--}}
                <td>
                    <a href="{{ route('scholarship.edit', $course->id) }}" class="btn btn-sm btn-primary">Edit</a>
                    <form id="delete-form-{{ $course->id }}" action="{{ route('scholarship.delete', $course->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
