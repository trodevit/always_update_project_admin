...
@section('content')
    <div class="container mt-5">
        ...
        <form action="{{route('honors.update',$upload->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            <input type="hidden" name="question" value="{{$upload->question}}">
                ...
                @if($subjects)
                    <select class="form-select" id="class" name="subject" required>
                        ...
                        @foreach($subjects as $subject)
                            <option value="{{$subject->id}}" {{ $upload->subject == $subject->id ? 'selected' : '' }}>{{$subject->subject}}</option>
                        @endforeach
                    </select>
                @else
                    <p class="text-danger">
            ...
            <a href="{{route('honors.index')}}" class="btn btn-info">All List</a>
        </form>
    </div>
@endsection
