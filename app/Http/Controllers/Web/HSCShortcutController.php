...
class HSCShortcutController extends Controller
{
    public function index()
    {
        $pdf = PDFCourse::where('p_d_f_courses.class_name','hsc')->where('types','technique')
            ->join('h_s_c_classes','h_s_c_classes.id','=','p_d_f_courses.hsc_year')
            ->select('p_d_f_courses.*','h_s_c_classes.class_name')
            ->get();
        return view('hsc.shortcut.index',['pdf' => $pdf]);
    }

    public function create()
    {
        $class = HSCClass::all();
        return view('hsc.shortcut.create',['class' => $class]);
    }
...
//        dd($request->all());
    ...
    public function edit($id)
    {
        ...
        $class = HSCClass::all();
        return view('hsc.shortcut.edit', ['pdf' => $pdf,'class'=>$class]);
    }
    ...
    public function update(Request $request, $id){
        try {
            ...
            return redirect()->route('hsc.shortcut');
        }
        catch (\Exception $exception){
            return response()->json($exception->getMessage());
        }
